...
class BThumbnail{
	...
	private $options = array(
		...
		"filter" => "",
		"sizefix" => false,
		"crop_posx" => BThumbnail::CROP_POSX_LEFT,
		"crop_posy" => BThumbnail::CROP_POSY_TOP,
		"quality" => 90,
		"watermark_path" => "",
		"watermark_pos" => BThumbnail::WM_RIGHT_BOTTOM,
		"watermark_padding" => 10,
	...
	private function build(){
		...
		//원본 불러오기
		$src_img = BImageutil::imageCreateFromPath($this->src_path);

		//원본 크기 및 타입 구하기
		list($src_ow, $src_oh, $this->image_type) = BImageutil::getImageSize($this->src_path);
		...
		//일반 축소이면
			...
			list($this->thumb_w, $this->thumb_h, $this->rate) = BImageutil::rateLimit(
		...
		//빈이미지 생성(dst_img)
		$this->dst_img = BImageutil::makeBlankImage(
	...
	public function printScreen(){
		BImageutil::imageOutput($this->image_type, $this->dst_img, "",
	...
	public function save(){
		BImageutil::imageOutput($this->image_type, $this->dst_img, $this->save_path,
			bt_varset($this->options["quality"]), bt_varset($this->options["fileter"]));
	}
	...
	public function pasteWatermark(){
		...
		//워터마크 이미지 불러와서 축소하기
		$wm_img = BImageutil::imageCreateFromPath($this->options["watermark_path"]);

		//썸네일 이미지리소스를 9등분으로 나눠서 좌표정보 얻기
		$info9 = BImageutil::getInfo9($this->dst_img);
	...
	public static function makeThumb($src_path, $thumb_dir, $w, $h, $options=array()){
		...
		$temp  = BFiledir::filename_parse($src_path);
		$fname = $temp["name"];
		$ext   = $temp["extension"];
