...
<div class="miso-post-garo">
	<div class="post-wrap<?php echo $is_dadan;?><?php echo ($caption == "3") ? ' is-hover' : ''; // 호버캡션 ?>">
		<?php // 이미지글
		for ($i=0; $i < $img_cnt; $i++) {
		...
		<div class="post-image post-col">
			<div class="post-item">
				<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
					<div class="img-wrap is-round-post-img" style="padding-bottom:<?php echo $img_h;?>%;">
						<?php echo $shadow_in;?>
						<?php if($caption && $wr_icon) { ?>
							<div class="in-left"><?php echo $wr_icon;?></div>
						<?php } ?>
						<?php if($img[$i]['as_list'] == "2" || $img[$i]['as_list'] == "3") echo $is_vicon; ?>
						<div class="img-item">
							<img src="<?php echo $img[$i]['img']['src'];?>" alt="<?php echo $img[$i]['img']['alt'];?>" class="wr-img">
							<?php if($is_caption && $caption) { ?>
								<div class="in-subject ellipsis trans-bg-black">
									<?php if($is_date || $img[$i]['comment']) { ?> 
				...
				<?php if($is_caption && !$caption) { // 캡션이 아닐 때 ?>
					<?php if($wset['line_height']) { ?>
						<div class="post-content<?php echo $is_center;?>">
							<div class="post-subject" style="height:<?php echo $wset['line_height'];?>px;">
								<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
									<?php echo $wr_icon;?>
									<?php echo $img[$i]['subject'];?>
									<?php if($is_cont) { ?>
					...
					<?php } else { ?>
						<div class="post-content">
							<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?> class="ellipsis">
								<?php if ($img[$i]['comment']) { ?>
									<span class="pull-right count orangered">+<?php echo $img[$i]['comment']; ?></span>
								<?php } ?>
								<?php echo $wr_icon;?>
								<?php echo $img[$i]['subject'];?>
							</a>
